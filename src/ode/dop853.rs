//! Explicit Runge-Kutta method with Dormand-Prince coefficients of order 8(5,3) and dense outputs of order 7.
...
use crate::ode::butcher_tableau::Dopri853;
use crate::ode::controller::Controller;
use crate::ode::dop_shared::*;
        ...
        DefaultAllocator: Allocator<T, D>,
{
    ...
    /// * `dx`      - Increment in the dense outputs. This argument has no effect if the outputs type is Sparse
    ...
    /// * `out_type`    - Type of the outputs. Must be a variant of the OutputType enum. Default is Dense
    ///
    ...
    /// If a dense outputs is required, computes the solution and pushes it into the outputs vector. Else, pushes the solution into the outputs vector.
    ...
    /// Getter for the independent variable's outputs.
    pub fn x_out(&self) -> &Vec<f64> {
        &self.x_out
    }

    /// Getter for the dependent variables' outputs.
