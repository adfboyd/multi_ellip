...
def write_summary(rows: list[dict[str, object]]) -> None:
    ...
    print("case          ndiv  dt     rows  end drift %  max |drift| %")
    for row in rows:
        print(
...
def main() -> None:
    ...
    for case in mesh_cases:
        data = results[str(case["name"])]
        marker_norm = np.sqrt(data[COL["ofx"]] ** 2 + data[COL["ofy"]] ** 2 + data[COL["ofz"]] ** 2)
        ax[2, 2].plot(data[COL["t"]], marker_norm - 1.0, color=colors[str(case["name"])], lw=1.1, label=f"ndiv={case['ndiv']}")
